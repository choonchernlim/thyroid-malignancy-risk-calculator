var app = function () {
    var b = 1, g = $( "#prevBtn" ), h = $( "#nextBtn" ), s = $( ".user-progress" ), t = $( ".user-screen" ), u = $( ".form" ), k = $( "#user-risk-text" ), l = $( "input:radio[name=user-risk-radio]" ), v = $( "input:radio[name=us-fna-radio]" ), r = $( "#computed-diagram" ), w = $( "#computed-value" ), x = $( "#malignancy-count-text" ), y = $( "#non-malignancy-count-text" ), e = null, n = null, z = function () {
        var a = [];
    ...
    return {
        start : function () {
            ...
            h.click( function () {
                var a = null;
                if ( 1 === b ) {
                    e ? B() : a = "Please specify a value for risk probability first.";
                }
                else if ( 2 === b ) {
                    if ( n ) {
                        var g = 0, c = "", c = z.compute( e,
                                n ), d = Math.round( c.postTestProbabilityEstimate ), f = 100 -
                                                                                          d, h = Math.round( c.postTestProbabilityLower ),
                            m = Math.round( c.postTestProbabilityHigher );
                        r.empty();
                        for ( var k = 0; 10 > k; ++k ) {
                            for ( var q = $( '<span style="line-height: 1">' ), l = 0; 10 > l; ++l ) {
                                c = g++ <
                                    d ?
                                    "red-text" :
                                    "teal-text", q.append( '<i class="small mdi-social-person ' + c + '"></i>' );
                            }
                            q.append( "<br/>" );
                            r.append( q )
                        }
                        x.text( d + " patient" + (1 != d ? "s" : "") + " with malignancy" );
